  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="rounded-md border-2 border-black/80 bg-white/55 p-1 hover:bg-white/80 transition-colors"
      >
        <X size={18} />
      </button>
    </div>
...
export const ToastProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <ToastContext.Provider value={{ showToast }}>
      {children}
      <div className="fixed bottom-[calc(6.25rem+env(safe-area-inset-bottom))] md:bottom-4 left-4 right-4 md:left-auto md:right-4 md:w-96 z-50 space-y-2">
        {toasts.map(toast => (
